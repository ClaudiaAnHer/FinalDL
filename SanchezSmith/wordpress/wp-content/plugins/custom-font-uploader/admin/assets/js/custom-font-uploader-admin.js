jQuery(document).ready(function () {
    ...
    //Verify API Key
    jQuery(document).on('click', '#cfup-verify-apikey', function(){
        var apikey = jQuery('#cfup-apikey').val();
        var btn_val = jQuery(this).val();
        jQuery('.cfup-admin-loader').show();
        var data = {
            'action': 'cfup_verify_apikey',
            'apikey': apikey,
        };
        jQuery.post(
            cfu_ajax_object.ajax_url,
            data,
            function (response) {
                jQuery('.cfup-admin-loader').hide();
                if( response == 'apikey-verified' ) {
                    jQuery('#cfup-verify-apikey').val('Verified');
                }

                if( response == 'apikey-unverified' ) {
                    jQuery('#cfup-verify-apikey').val('Not verified');
                }
            }
        );
    });
